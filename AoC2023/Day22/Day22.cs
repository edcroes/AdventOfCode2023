...
public class Day22 : IMDay
{
    ...
    public async Task<string> GetAnswerPart1()
    {
        ...
        if (_supporting is null || _supportedBy is null)
            (_supporting, _supportedBy) = GetFallenStack(lines);
    ...
    public async Task<string> GetAnswerPart2()
    {
        ...
        if (_supporting is null || _supportedBy is null)
            (_supporting, _supportedBy) = GetFallenStack(cuboids);
    ...
    private static (Dictionary<Line3D, List<Line3D>> supporting, Dictionary<Line3D, Line3D[]> supportedBy) GetFallenStack(IEnumerable<Line3D> lines)
    {
        List<Line3D> fallen = [new(new(-1, -1, -1), new(-1, -1, -1))];
